...
**Mesures appliquées :**
- **Rate limiting** sur `POST /auth/login` : 5 tentatives par minute par IP via `rate_limit_middleware` (Map en mémoire, fenêtre glissante d'1 minute). Réponse HTTP 429 au dépassement.
- Access token de courte durée (15 min) ; expiration gérée côté backend.
- Refresh token révocable : la déconnexion (`POST /auth/logout`) révoque le token en base (`revokedAt = now()`).
...
| A07 | Authentication Failures | ✅ Couvert | Rate limiting, refresh token révocable |
